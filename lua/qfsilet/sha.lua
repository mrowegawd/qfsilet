	...
	function sha256_feed_64(H, str, offs, size)
		...
		for pos = offs, offs + size - 1, 64 do
			...
			for j = 0, 63, 8 do -- Thanks to Peter Cawley for this workaround (unroll the loop to avoid "PHI shuffling too complex" due to PHIs overlap)
				local z =
					NORM(XOR(g, AND(e, XOR(f, g))) + XOR(ROR(e, 6), ROR(e, 11), ROL(e, 7)) + (W[j] + K[j + 1] + h))
	...
	function keccak_feed(lanes_lo, lanes_hi, str, offs, size, block_size_in_bytes)
		...
		for pos = offs, offs + size - 1, block_size_in_bytes do
			...
			for round_idx = 1, 24 do
				for j = 1, 5 do
					lanes_lo[25 + j] =
						XOR(lanes_lo[j], lanes_lo[j + 5], lanes_lo[j + 10], lanes_lo[j + 15], lanes_lo[j + 20])
				end
				for j = 1, 5 do
					lanes_hi[25 + j] =
						XOR(lanes_hi[j], lanes_hi[j + 5], lanes_hi[j + 10], lanes_hi[j + 15], lanes_hi[j + 20])
				end
		...
		function blake3_feed_64(str, offs, size, flags, chunk_index, H_in, H_out, wide_output, block_length)
			...
			for pos = offs, offs + size - 1, 64 do
				...
				v[0x8], v[0x9], v[0xA], v[0xB] =
					NORM(sha2_H_hi[1]), NORM(sha2_H_hi[2]), NORM(sha2_H_hi[3]), NORM(sha2_H_hi[4])
	...
	function blake2b_feed_128(H_lo, H_hi, str, offs, size, bytes_compressed, last_block_size, is_last_node)
		...
		for pos = offs, offs + size - 1, 128 do
			...
			for j = 1, 12 do
				...
				v4_lo, v4_hi =
					(v4_lo - z_lo) / 2 ^ 24 % 2 ^ 8 + z_hi * 2 ^ 8, (v4_hi - z_hi) / 2 ^ 24 % 2 ^ 8 + z_lo * 2 ^ 8
				k = row[2] * 2
				...
				v4_lo, v4_hi =
					z_lo * 2 ^ 1 + (v4_hi - z_hi) / 2 ^ 31 % 2 ^ 1, z_hi * 2 ^ 1 + (v4_lo - z_lo) / 2 ^ 31 % 2 ^ 1
				k = row[3] * 2
				...
				v5_lo, v5_hi =
					(v5_lo - z_lo) / 2 ^ 24 % 2 ^ 8 + z_hi * 2 ^ 8, (v5_hi - z_hi) / 2 ^ 24 % 2 ^ 8 + z_lo * 2 ^ 8
				k = row[4] * 2
				z = v1_lo % 2 ^ 32 + v5_lo % 2 ^ 32 + W[k - 1]
				v1_lo = z % 2 ^ 32
				v1_hi = v1_hi + v5_hi + (z - v1_lo) / 2 ^ 32 + W[k]
				...
				v5_lo, v5_hi =
					z_lo * 2 ^ 1 + (v5_hi - z_hi) / 2 ^ 31 % 2 ^ 1, z_hi * 2 ^ 1 + (v5_lo - z_lo) / 2 ^ 31 % 2 ^ 1
				k = row[5] * 2
				...
				v6_lo, v6_hi =
					(v6_lo - z_lo) / 2 ^ 24 % 2 ^ 8 + z_hi * 2 ^ 8, (v6_hi - z_hi) / 2 ^ 24 % 2 ^ 8 + z_lo * 2 ^ 8
				k = row[6] * 2
				z = v2_lo % 2 ^ 32 + v6_lo % 2 ^ 32 + W[k - 1]
				v2_lo = z % 2 ^ 32
				v2_hi = v2_hi + v6_hi + (z - v2_lo) / 2 ^ 32 + W[k]
				...
				v6_lo, v6_hi =
					z_lo * 2 ^ 1 + (v6_hi - z_hi) / 2 ^ 31 % 2 ^ 1, z_hi * 2 ^ 1 + (v6_lo - z_lo) / 2 ^ 31 % 2 ^ 1
				k = row[7] * 2
				...
				v7_lo, v7_hi =
					(v7_lo - z_lo) / 2 ^ 24 % 2 ^ 8 + z_hi * 2 ^ 8, (v7_hi - z_hi) / 2 ^ 24 % 2 ^ 8 + z_lo * 2 ^ 8
				k = row[8] * 2
				z = v3_lo % 2 ^ 32 + v7_lo % 2 ^ 32 + W[k - 1]
				v3_lo = z % 2 ^ 32
				v3_hi = v3_hi + v7_hi + (z - v3_lo) / 2 ^ 32 + W[k]
				...
				v7_lo, v7_hi =
					z_lo * 2 ^ 1 + (v7_hi - z_hi) / 2 ^ 31 % 2 ^ 1, z_hi * 2 ^ 1 + (v7_lo - z_lo) / 2 ^ 31 % 2 ^ 1
				k = row[9] * 2
		...
		if message_part then
			...
			if tail then
				...
				if not XOF_length or B2_offset then
					...
					H = sub(
						gsub(table_concat(H, "", 1, max_reg), "(..)(..)(..)(..)", "%4%3%2%1"),
						1,
		...
		if message_part then
			if tail then
				...
				if tail ~= "" and #tail + #message_part > 128 then
					offs = 128 - #tail
					bytes_compressed =
						blake2b_feed_128(H_lo, H_hi, tail .. sub(message_part, 1, offs), 0, 128, bytes_compressed)
					tail = ""
				end
				local size = #message_part - offs
	...
	local function partial(message_part)
		if message_part then
			...
			if instances then
				...
				result = sub(
					gsub(table_concat(root_H, "", 1, max_reg), "(..)(..)(..)(..)", "%4%3%2%1"),
					1,
					digest_size_in_bytes * 2
				)
			end
